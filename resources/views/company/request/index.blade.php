                ...
                <div class="card-body">
                    ...
                    <p class="card-text" style="direction: rtl; text-align: right;">
                        ...
                        @if($data->category && $data->category->name)
                            <span class="badge bg-success me-2 mt-2 float-start">{{ $data->category->name }}</span>
                        @else
                            <span class="badge bg-secondary me-2 mt-2 float-start">No Category</span>
                        @endif
                    </p>
                    <p class="mt-2" style="color: black; direction: rtl; text-align: right;"><strong class="text-change">مبلغ التمويل</strong> <br> SAR {{ $data->total_funds ?? '' }}</p>
                    <p class="mt-2" style="color: black; direction: rtl; text-align: right;"><strong class="text-change">الغرض من التمويل</strong> <br> {{ $data->purpose_of_funding ?? '' }}</p>
                    <p class="mt-2" style="color: black; direction: rtl; text-align: right;"><strong class="text-change">الوصف</strong> <br> {{ $data->description ?? '' }}</p>
                    <div class="d-flex flex-row justify-content-between" style="direction: rtl;">
                        @if($data->status == 1)
                        <span class="badge bg-success me-2 float-start">موافقة</span>
                        @else
                        <span class="badge bg-warning me-2 float-start">قيد المراجعة</span>
                        @endif
                        <a href="#" data-bs-toggle="modal" data-bs-target="#modalId{{ $data->id }}" class="fa fa-file btn-sm btn-red mr-3" style="font-size: 15px; background-color: #214D45; color: white;"></a>
                    </div>
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="tab-content" id="tabContent{{ $data->id }}">
                                        ...
                                        <div class="tab-pane fade show active" id="form-{{ $data->id }}" role="tabpanel" aria-labelledby="form-tab-{{ $data->id }}">
                                            <form class="mt-3" action="{{ route('company.request.updateDocument') }}" enctype="multipart/form-data" method="post">
                                                @csrf
                                                <label for="">اسم التحديث</label>
                                                <input type="text" class="form-control @error('update_name') is-invalid @enderror" name="update_name" placeholder="اسم التحديث" id="">
                                                @error('update_name')
                                                <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                                ...
                                                <input type="text" class="form-control" value="{{ $data->name }}" readonly id="">
                                                <br>
                                                <label for="">مستند التحديث</label>
                                                <input type="file" class="form-control @error('update_name') is-invalid @enderror" name="document[]" multiple>
                                                @error('document')
                                                <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                                @enderror
                                                <br>
                                                <button type="button" class="btn-sm btn-secondary" data-bs-dismiss="modal">إغلاق</button>
                                        ...
                                        <div class="tab-pane fade" id="table-{{ $data->id }}" role="tabpanel" aria-labelledby="table-tab-{{ $data->id }}">
                                            <div class="row mt-3">
                                                ...
                                                @forelse ($data->projectUpdate as $projectData)
                                                <div class="col-lg-6 p-2" style="border: 1px solid;">
                                                    {{ $projectData->update_name ?? 'لم يتم تقديم اسم التحديث' }}
                                                </div>
                                                <div class="col-lg-6 p-2" style="border: 1px solid;">
                                                    @php
                                                    $document = json_decode($projectData->document, true) ?? [];
