    ...
    <div class="row">
        <div class="card p-1">
            <div class="card-header bg-primary p-2 d-flex flex-row">
                <h3 class="text-white">Edit Assigned Company</h3>
            </div>

            <div class="card-body p-2">
                <form method="POST" action="{{ route('admin.assigned.company.update', $assignedCompany->id) }}">
                    @csrf
                    <div class="row">
                        <div class="mb-3 mt-2 col-lg-12">
                            ...
                            <select name="category_id" id="category_id" class="form-control @error('category_id') is-invalid @enderror">
                                ...
                                <option value="{{ $category->id }}"
                                    {{ old('category_id', $assignedCompany->category_id) == $category->id ? 'selected' : '' }}>
                                    {{ $category->name ?? '' }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3 mt-2 col-lg-12">
                            ...
                            <select name="company_id" class="form-control @error('company_id') is-invalid @enderror">
                                ...
                                <option value="{{ $company->id }}"
                                    {{ old('company_id', $assignedCompany->company_id) == $company->id ? 'selected' : '' }}>
                                    {{ $company->name ?? '' }}
                                </option>
                                @endforeach
                            </select>
